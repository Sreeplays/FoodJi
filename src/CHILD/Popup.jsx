...
const Popup = ({closeBtn, detail, addedCart}) => {
 const allMenu = useContext(AllMenuContext)
  ...
  const popItem = allMenu.filter((catItem) => {
    ...
    return (
      ...
      <button class="absolute bg-[#ffb902] p-3 sm:bottom-[8rem] bottom-[8rem] sm:right-[9rem] right-[4rem] rounded-lg mb-5 hover:bg-green-300 delay-300" onClick={() => addedCart(catItem.strMeal, catItem.strMealThumb)}>Add to Cart</button>
      </div>
      <h4 className="text-center top-[360px] absolute text-white font-mono font-extrabold text-xl pb-4">Ingredients</h4>
      <ul className="items-center justify-center bottom-[13rem] absolute flex sm:grid-rows-4 grid-rows-2 gap-3 mb-4 mt-4">
        <li className="px-4 text-black rounded-lg bg-gray-200 py-2 gap-3 bg-red-500"> {catItem.strIngredient1} </li>
        <li className="px-4 text-black rounded-lg bg-gray-200 py-2 gap-3 bg-red-500"> {catItem.strIngredient2} </li>
        <li className="px-4 text-black rounded-lg bg-gray-200 py-2 gap-3 bg-red-500"> {catItem.strIngredient3} </li>
        <li className="px-4 text-black rounded-lg bg-gray-200 py-2 gap-3 bg-red-500"> {catItem.strIngredient4} </li>
      </ul>
